...
libevdev_new(void) {
    ...
    dev = calloc(1, sizeof(*dev));  //申请内存空间，并初始化
    if (!dev) //如果分配失败（由于内存不足导致）
        return NULL; //则创建失败，返回NULL

    libevdev_reset(dev); //对结构体的变量进行重置

    return dev;
}

LIBEVDEV_EXPORT int
libevdev_new_from_fd(int fd, struct libevdev **dev) //二级指针
{
    struct libevdev *d; // 创建一个临时指针
    int rc;  // 返回值

    d = libevdev_new(); //为 libevdev 申请内存并返回指针对象，赋值给指针变量 d
    if (!d)
        return -ENOMEM;

    rc = libevdev_set_fd(d, fd); //d 为 libevdev 的指针变量，fd 为对应设备的文件描述符
    if (rc < 0)
        libevdev_free(d);
    else
        *dev = d;
...
libevdev_set_fd(struct libevdev *dev, int fd) {
    ...
    if (dev->initialized) { //判断dev是否已经被初始化过
        log_bug(dev, "device already initialized.\n");
        return -EBADF;
    } else if (fd < 0) //判断文件描述符是否正确
        return -EBADF;

    libevdev_reset(dev); //对dev的结构体变量进行重置
    //ioctl(input/output control)是一个专用于设备输入输出操作的系统调用, 获取/dev/input目录下的eventX设备支持哪些事件
    rc = ioctl(fd, EVIOCGBIT(0, sizeof(dev->bits)), dev->bits);
    if (rc < 0)
        goto out;

    memset(buf, 0, sizeof(buf)); //对数组字符数组进行初始化
    rc = ioctl(fd, EVIOCGNAME(sizeof(buf) - 1), buf); //通过ioctl命令EVIOCGNAME，能获取dev/input/event*对应的Device Name
    if (rc < 0)
        goto out;

    free(dev->name);
    dev->name = strdup(buf); //对缓区的字符进行拷贝
    if (!dev->name) {
        errno = ENOMEM;
        goto out;
    }
...
LIBEVDEV_EXPORT const struct input_absinfo *  //绝对设备的输入信息
libevdev_get_abs_info(const struct libevdev *dev, unsigned int code) { //dev是设备的结构体指针，code是设备的type值
    if (!libevdev_has_event_type(dev, EV_ABS) ||
        !libevdev_has_event_code(dev, EV_ABS, code))
        return NULL;
